export const COMPACT_SAVE_FORMAT_ID = "pi4c";
...
function encodeRouteTuple(saveData, routeIdOrder, options = {}) {
  ...
  return [currentIndex, furthestUnlockedIndex, sparsePairs];
}
...
function decodeRouteTuple(routeTuple, routeIdOrder, defaultRouteId, baseCounts, options = {}) {
  ...
  const furthestUnlockedIndex = Math.min(
    ...
    clampNonNegativeInt(rawTuple[1], 0, options),
  );
  ...
  const unlockedRouteIds = orderedRouteIds.slice(0, furthestUnlockedIndex + 1);
