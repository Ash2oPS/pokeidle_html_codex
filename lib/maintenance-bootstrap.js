...
function isScheduledMaintenanceActive(config, nowDate = new Date()) {
  ...
  return config.weeklyWindows.some((windowConfig) => (
...
export function shouldShowMaintenanceScreen({
  ...
  if (production) {
    ...
    if (shouldUseScheduledMaintenance(config)) {
      return isScheduledMaintenanceActive(config, nowDate);
    }
