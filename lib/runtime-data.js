...
const routeAccessRulesSchema = z
  .object({
    requires_flags_all: z.array(z.string().min(1)).optional().default([]),
    requires_flags_any: z.array(z.string().min(1)).optional().default([]),
    blocked_reason_fr: z.string().optional().default(""),
  })
  .passthrough();

const zoneActionSchema = z
  .object({
    action_id: z.string().min(1),
    label_fr: z.string().min(1),
    kind: z.enum(["dialogue", "trainer_battle"]).optional().default("dialogue"),
    dialogue_id: z.string().optional().default(""),
    trainer_battle_id: z.string().optional().default(""),
    desktop_anchor_pct: zoneAnchorPctSchema.optional().default({ x: 50, y: 50 }),
    mobile_anchor_pct: zoneAnchorPctSchema.optional().default({ x: 50, y: 50 }),
  })
  .superRefine((value, ctx) => {
    const kind = String(value?.kind || "dialogue").trim();
    if (kind === "dialogue" && !String(value?.dialogue_id || "").trim()) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "dialogue_id requis pour une action de type dialogue",
        path: ["dialogue_id"],
      });
    }
    if (kind === "trainer_battle" && !String(value?.trainer_battle_id || "").trim()) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "trainer_battle_id requis pour une action de type trainer_battle",
        path: ["trainer_battle_id"],
      });
    }
  })
  .passthrough();

const trainerBattleRosterEntrySchema = z
  .object({
...
const trainerBattlePayloadSchema = z
  .object({
    trainer_battle_id: z.string().min(1),
    trainer_name_fr: z.string().min(1),
    route_id: z.string().min(1),
    victory_flag_id: z.string().min(1),
    roster: z.array(trainerBattleRosterEntrySchema).min(1),
  })
  .passthrough();
...
const routeDataSchema = z
  .object({
    ...
    connected_route_ids: z.array(z.string().min(1)).optional().default([]),
    arrival_dialogue_ids_once: z.array(z.string().min(1)).optional().default([]),
    zone_actions: z.array(zoneActionSchema).optional().default([]),
    access_rules: routeAccessRulesSchema.optional().default({}),
    encounters: z.array(routeEncounterSchema).optional().default([]),
  })
  .passthrough();

const dialogueEffectSchema = z
  .object({
    kind: z.enum(["set_flag_true", "set_flag_false"]),
...
const dialogueChoiceSchema = z
  .object({
    ...
    requires_flags_all: z.array(z.string().min(1)).optional().default([]),
    requires_flags_any: z.array(z.string().min(1)).optional().default([]),
    effects: z.array(dialogueEffectSchema).optional().default([]),
  })
  .passthrough();
...
const dialogueNodeSchema = z
  .object({
    ...
    speaker_fr: z.string().optional().default(""),
    text_fr: z.string().min(1),
    next_node_id: z.string().optional().default(""),
    choices: z.array(dialogueChoiceSchema).optional().default([]),
    effects_on_enter: z.array(dialogueEffectSchema).optional().default([]),
  })
  .passthrough();

const dialoguePayloadSchema = z
  .object({
...
const pokemonPayloadSchema = z
  .object({
    ...
    attack_mode: z.enum(["projectile", "projectiles", "laser"]).optional(),
...
export function validateDialoguePayload(payload, contextLabel = "Dialogue data") {
  return parseWithSchema(dialoguePayloadSchema, payload, contextLabel);
}

export function validateTrainerBattlePayload(payload, contextLabel = "Trainer battle data") {
  return parseWithSchema(trainerBattlePayloadSchema, payload, contextLabel);
}
