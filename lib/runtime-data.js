...
const saveBridgeResponseSchema = z
  .object({
    save: rawSaveSchema.nullable().optional(),
  })
  .passthrough();
...
export function parseSaveBridgePayload(payload, contextLabel = "Save bridge") {
  const parsed = parseWithSchema(saveBridgeResponseSchema, payload, contextLabel);
  if (!parsed.save) {
    return null;
  }
  return assertSaveObject(parsed.save, `${contextLabel} payload.save`);
}
