...
function isLocalUpdatePreviewLocation(location = window.location) {
...
export function initializeGithubUpdateChecker({ currentVersion, checkIntervalMs = UPDATE_RECHECK_INTERVAL_MS } = {}) {
  ...
  if (isLocalUpdatePreviewLocation()) {
    // Dev/test-only preview hook for visual galleries.
    window.__pokeidle_preview_update_required = (previewRemoteVersion = "", previewLocalVersion = localVersion) => {
      const nextLocalVersion = String(previewLocalVersion || localVersion).trim() || localVersion;
      const nextRemoteVersion = String(previewRemoteVersion || `${localVersion}-preview`).trim() || `${localVersion}-preview`;
      hasShownUpdateModal = true;
      showUpdateRequiredModal(nextLocalVersion, nextRemoteVersion);
      return true;
    };
  }
