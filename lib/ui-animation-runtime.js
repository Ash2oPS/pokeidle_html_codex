...
export function createUiAnimationRuntime({
  ...
  function showLoadingScreen(message = loadingScreenDefaultText, options = {}) {
    if (!loadingScreenEl) {
      return;
    }
    const disablePokeballSpin = options?.disablePokeballSpin === true;
    clearLoadingScreenHideTimer();
    setLoadingScreenMessage(message);
    loadingScreenEl.classList.toggle("is-static-pokeball", disablePokeballSpin);
  ...
  function hideLoadingScreen(options = {}) {
    ...
    clearLoadingScreenHideTimer();
    if (immediate) {
      loadingScreenEl.classList.remove("is-static-pokeball");
