...
export function normalizeTalentDefinition(rawTalent) {
  ...
  const nameFr = String(source.nameFr ?? source.name_fr ?? source.talent_name_fr ?? source.talentNameFr ?? defaultName).trim();
  const nameEn = String(source.nameEn ?? source.name_en ?? source.talent_name_en ?? source.talentNameEn ?? defaultName).trim();
