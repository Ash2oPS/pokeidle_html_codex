...
export function createZoneDialogueRuntime({
  ...
  triggerTrainerBattleAction = () => false,
} = {}) {
  ...
  function getCurrentRouteZoneActions() {
    ...
    return actions.filter((action) => {
      if (!action || !String(action.action_id || "").trim()) {
        return false;
      }
      const kind = String(action.kind || "dialogue").trim();
      if (kind === "trainer_battle") {
        return Boolean(String(action.trainer_battle_id || "").trim());
      }
      return Boolean(String(action.dialogue_id || "").trim());
    });
  }
  ...
  async function loadDialogueDefinition(dialogueId) {
    ...
    const task = fetchFn(buildDialogueDataPath(id))
      .then(async (response) => {
        if (!response.ok) {
  ...
  function triggerZoneAction(actionId) {
    ...
    if (String(zoneAction.kind || "dialogue").trim() === "trainer_battle") {
      return Boolean(triggerTrainerBattleAction(String(zoneAction.trainer_battle_id || "").trim(), {
        routeId,
        sourceActionId: id,
      }));
    }
