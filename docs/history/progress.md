...
## Additional progress (2026-03-23, visual-ready gallery hardening + route navigation extraction)
- Hardened the gallery and screenshot contract around actual visual readiness instead of loose runtime readiness:
  - `systems/ui/runtime-ui-interaction-system.js` now exposes `boot_phase`, `loading_overlay_visible`, `visual_ready`, and a compact `route_nav_state` in `render_game_to_text`;
  - `lib/maintenance-bootstrap.js` now exports the same boot/capture fields for maintenance mode so visual tooling keeps a stable contract in both paths;
  - screenshot capture waits now use explicit rendered-state checks before writing images, preventing captures while the loading overlay is still visible.
- Reworked the gallery scenarios to be state-driven instead of timing-driven:
  - `scripts/testing/playwright/actions/desktop-ui-gallery.json` now waits for `visual_ready`, starter modal state, and route-navigation state before capturing;
  - `scripts/testing/playwright/actions/mobile-ui-gallery.json` follows the same state-driven flow for portrait mobile;
  - optional gallery steps can now be skipped intentionally when their state preconditions are absent, instead of crashing on selectors that were never legitimate in the current run.
- Extracted the route navigation logic out of the monolithic runtime in two focused modules:
  - `lib/route-navigation-runtime.js` owns the route connectivity/access view-model calculations;
  - `systems/ui/route-navigation-ui.js` owns the route navigation DOM building and rendering;
  - `game-runtime.js` now delegates route navigation state and rendering to those modules instead of carrying the full implementation inline.
- Continued the runtime split for the remaining zone flows:
  - `lib/zone-dialogue-runtime.js` now owns the dialogue queue/loading/open/choice/close logic and zone-action dialogue orchestration;
  - `systems/ui/zone-dialogue-ui.js` now owns zone-action button rendering and dialogue modal DOM rendering;
  - `systems/ui/map-navigation-ui.js` now owns map marker and connected-route panel rendering;
  - `game-runtime.js` keeps the orchestration hooks and modal gating, but no longer carries the full inline implementation of those flows.
- Fixed a real WebKit boot regression introduced during extraction:
  - the new dialogue runtime wiring initially captured runtime-ui facade bindings before their late destructuring was initialized;
  - the boot path now uses lazy callbacks for those late-bound dependencies, restoring iPhone WebKit startup and keeping `tests/runtime-ios-touch-context-menu.test.mjs` green.
- Fixed the route lock-info contract so galleries match the real progression rules:
  - `route_nav_state.hasBlockedDestinations` and `route_nav_state.hasInfoPanelTarget` are now derived from connected-but-not-yet-unlocked routes, not only from access-flag hard blocks;
  - this keeps the lock-info scenario valid for legitimate progression-gated exits such as early Route 1 to Viridian.
- Polished the gallery captures to avoid noisy temporary notifications in the key idle screenshots:
  - desktop and mobile gallery scenarios now wait for `notifications_temporary: 0` before the main route-idle captures;
  - the resulting idle and route-drawer screenshots are clean enough to serve as regression references.
- Added focused automated coverage:
  - `tests/route-navigation-runtime.test.mjs`
  - `tests/zone-dialogue-runtime.test.mjs`
  - expanded `tests/runtime-ui-interaction-system.test.mjs`
  - expanded `tests/maintenance-bootstrap.test.mjs`
- Validation:
  - `node --check game-runtime.js` -> PASS
  - `node --check scripts/testing/playwright/web-game-playwright-client.mjs` -> PASS
  - `node --test tests/maintenance-bootstrap.test.mjs tests/route-navigation-runtime.test.mjs tests/runtime-ui-interaction-system.test.mjs` -> PASS
  - `node --test tests/runtime-ios-touch-context-menu.test.mjs` -> PASS
  - `node --test tests/maintenance-bootstrap.test.mjs tests/route-navigation-runtime.test.mjs tests/zone-dialogue-runtime.test.mjs tests/runtime-ui-interaction-system.test.mjs` -> PASS
  - `npm test` -> PASS
  - `npm run test:visual:gallery:desktop` -> PASS
  - `npm run test:visual:gallery:mobile` -> PASS
- Visual artifacts reviewed manually:
  - desktop starter modal: `output/ui-state-gallery/desktop-landscape/starter-modal.png`
  - desktop route idle: `output/ui-state-gallery/desktop-landscape/idle.png`
  - desktop route lock info: `output/ui-state-gallery/desktop-landscape/route-nav-lock-info.png`
  - desktop map and lock info: `output/ui-state-gallery/desktop-landscape/map.png`, `output/ui-state-gallery/desktop-landscape/map-lock-info.png`
  - desktop shop and gacha: `output/ui-state-gallery/desktop-landscape/shop.png`, `output/ui-state-gallery/desktop-landscape/gacha.png`
  - mobile starter modal: `output/ui-state-gallery/mobile-portrait/starter-modal.png`
  - mobile route idle and drawer: `output/ui-state-gallery/mobile-portrait/idle.png`, `output/ui-state-gallery/mobile-portrait/route-nav-drawer.png`
  - mobile lock info, menu, map, shop: `output/ui-state-gallery/mobile-portrait/route-nav-lock-info.png`, `output/ui-state-gallery/mobile-portrait/menu.png`, `output/ui-state-gallery/mobile-portrait/map.png`, `output/ui-state-gallery/mobile-portrait/shop.png`
- Added a dev-only quick-level button in the bottom-right HUD:
  - `+1 niv` now appears only outside the production GitHub Pages runtime.
  - Clicking it grants +1 level to every owned Pokemon, including the active team and boxed Pokemon.
  - The action preserves existing progression hooks by refreshing team sync, level-up feedback, appearance unlocks, and evolution-ready notifications.
- Wired the new dev action through the runtime UI contracts:
  - `systems/ui/runtime-ui-dom-factory.js` exposes the button and stable runtime id.
  - `systems/ui/runtime-input-system.js` forwards the click into the UI interaction facade.
  - `systems/ui/runtime-ui-interaction-system.js` applies the bulk level-up across owned entities and refreshes open collection views.
  - `game-runtime.js` toggles the control strictly in dev mode.
- Added focused regression coverage:
  - `tests/runtime-ui-dom-factory.test.mjs`
  - `tests/runtime-input-system.test.mjs`
  - `tests/runtime-ui-interaction-system.test.mjs`
- Validation:
  - `node --test tests/runtime-ui-dom-factory.test.mjs tests/runtime-input-system.test.mjs tests/runtime-ui-interaction-system.test.mjs tests/version-environment.test.mjs` -> PASS
  - `npm run test:visual:gallery:desktop` -> PASS
  - `npm run test:visual:gallery:mobile` -> PASS
- Visual artifacts reviewed manually:
  - desktop idle with dev button: `output/ui-state-gallery/desktop-landscape/idle.png`
  - mobile idle with dev button: `output/ui-state-gallery/mobile-portrait/idle.png`
  - mobile fullscreen menu with dev button: `output/ui-state-gallery/mobile-portrait/menu.png`
- Follow-up fix for the dev quick-level button after runtime repro:
  - reproduced the real browser failure and confirmed the click path crashed with `TypeError: setEntityLevel is not a function`;
  - fixed `game-runtime.js` so the UI interaction system receives the dev boost bindings explicitly in the runtime snapshot;
  - hid the dev button while starter/tutorial blocking modals are open so it no longer looks clickable when the overlay would intercept input.
- Added targeted regression coverage:
  - `tests/runtime-dev-level-button.test.mjs`
- Validation:
  - `node --test tests/runtime-ui-dom-factory.test.mjs tests/runtime-input-system.test.mjs tests/runtime-ui-interaction-system.test.mjs tests/runtime-dev-level-button.test.mjs tests/version-environment.test.mjs` -> PASS
  - targeted browser repro after fix: button click levels the starter from lvl 1 to lvl 2 with the on-screen `Boost dev` feedback
  - `npm run test:visual:gallery:desktop` -> PASS
  - `npm run test:visual:gallery:mobile` -> PASS
- Visual artifacts reviewed manually after the fix:
  - desktop starter modal without the dev button: `output/ui-state-gallery/desktop-landscape/starter-modal.png`
