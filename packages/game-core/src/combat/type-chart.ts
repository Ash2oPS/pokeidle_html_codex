const typeChart: Record<string, Partial<Record<string, number>>> = {
  ...
  ice: {
    fire: 0.5, water: 0.5, grass: 2, ground: 2, flying: 2, dragon: 2, steel: 0.5, ice: 0.5,
  },
  fighting: {
...
export function getTypeMultiplier(
  ...
  const attackRow = typeChart[attackType];

  return defensiveTypes.reduce((multiplier, defensiveType) => {
    const value = attackRow?.[defensiveType] ?? 1;
