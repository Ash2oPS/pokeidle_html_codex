...
function readPngDimensions(imagePath) {
  ...
  assert.equal(buffer.subarray(0, 8).toString("hex"), pngSignature, `${imagePath} must be a PNG image`);
  return {
...
test("Johto zones use Johto-specific non-placeholder background assets", () => {
  const catalog = readJson(JOHTO_CATALOG_PATH);
  const johtoZoneOrder = Array.isArray(catalog?.zone_order) ? catalog.zone_order : [];
  ...
  for (const zoneId of johtoZoneOrder) {
    const zonePath = path.join(MAP_DATA_DIR, `${zoneId}.json`);
    const payload = validateRouteDataPayload(readJson(zonePath), zoneId);
    const backgroundPath = String(payload.background_image || "").trim();
    assert.match(
      backgroundPath,
      /^assets\/backgrounds\/johto_[a-z0-9_]+_hgss\.png$/,
      `${zoneId} should use a Johto HGSS background asset`,
    );
    assert.ok(!backgroundPath.includes("placeholder"), `${zoneId} should not reference placeholder-named art`);
    assert.ok(!backgroundPath.includes("kanto_"), `${zoneId} should not fall back to a Kanto background`);

    if (payload.zone_type !== "route") {
      const absoluteBackgroundPath = resolveWorkspacePath(backgroundPath);
      const { width, height } = readPngDimensions(absoluteBackgroundPath);
      assert.notDeepEqual(
        { width, height },
        { width: 960, height: 640 },
        `${zoneId} should no longer use the fixed Johto placeholder canvas`,
      );
    }
  }
});
