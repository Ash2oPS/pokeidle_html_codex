...
function notifyFirstTimeSpeciesProgress(pokemonId, kind, isShiny, previousValue, nextValue) {
  ...
  const pokemonName = getPokemonProgressDisplayNameById(pokemonId);
...
function getPokemonProgressDisplayNameById(pokemonId) {
  const defaultName = String(getPokemonDisplayNameById(pokemonId) || "");
  if (!/^Pokemon\s+#?\d+$/i.test(defaultName.trim())) {
    return defaultName;
  }

  const speciesNameEn = String(getPokemonEntityRecord(pokemonId)?.species_name_en || "").toLowerCase().trim();
  if (!speciesNameEn) {
    return defaultName;
  }

  const fallbackName = speciesNameEn
    .split(/[_\s-]+/g)
    .filter(Boolean)
    .map((segment) => segment.charAt(0).toUpperCase() + segment.slice(1))
    .join(" ");
  return normalizeUiDisplayText(fallbackName || defaultName, { frenchTypography: true });
}
...
function removeMapMarkerButton(routeId) {
  const button = mapMarkerButtonsByRouteId.get(routeId);
  if (!button) {
    return;
  }
  button.remove();
...
function maybeTriggerMewtwoRenameEasterEgg(pokemonId, nickname) {
  const nextNickname = sanitizePokemonNickname(nickname);
  const lugiaShinyAlreadyUnlocked =
    Math.max(0, toSafeInt(getPokemonEntityRecord(LUGIA_RENAME_EASTER_EGG_UNLOCK_POKEMON_ID)?.captured_shiny, 0)) > 0;
  const shouldUnlock = shouldUnlockLugiaShinyFromMewtwoRename({
    pokemonId,
    nickname: nextNickname,
    alreadyUnlocked: lugiaShinyAlreadyUnlocked,
  });
  if (!shouldUnlock) {
    return {
      triggered: false,
  ...
  const record = ensureSpeciesStats(LUGIA_RENAME_EASTER_EGG_UNLOCK_POKEMON_ID);
  if (!isEntityUnlocked(record)) {
    setEntityLevel(record, 1);
    record.xp = 0;
    markEntityUnlocked(record, true);
  }
  if (!String(record.species_name_en || "").trim()) {
    record.species_name_en = "lugia";
  }

  reconcileAppearanceForEntityRecord(record, LUGIA_RENAME_EASTER_EGG_UNLOCK_POKEMON_ID);

  if (Math.max(0, toSafeInt(record.encountered_shiny, 0)) <= 0) {
    incrementSpeciesStat(LUGIA_RENAME_EASTER_EGG_UNLOCK_POKEMON_ID, "encountered", true, 1);
  }
  if (Math.max(0, toSafeInt(record.captured_shiny, 0)) <= 0) {
