...
function queueEvolutionAnimationForResult(evolutionResult) {
  ...
  const fireworksStartMs = Math.max(0, EVOLUTION_ANIM_TOTAL_MS - EVOLUTION_ANIM_REVEAL_MS);
  const fireworksWindowMs = Math.max(1, EVOLUTION_ANIM_REVEAL_MS);
  for (let i = 0; i < particleCount; i += 1) {
    particles.push({
      startMs: randomRange(fireworksStartMs, fireworksStartMs + fireworksWindowMs * 0.55),
      durationMs: randomRange(
        Math.max(320, Math.floor(fireworksWindowMs * 0.5)),
        Math.max(520, fireworksWindowMs),
      ),
      baseAngle: randomRange(0, Math.PI * 2),
      spinTurns: randomRange(-0.16, 0.16),
      radiusStart: randomRange(0.04, 0.12),
      radiusGrow: randomRange(0.3, 0.74),
      lift: randomRange(0.02, 0.18),
      heightOffset: randomRange(-0.08, 0.08),
      size: randomRange(1.8, 4.1),
      color: [255, 255, 255],
    });
  }
...
function showPanelBlockMessage(reason) {
  if (state.simulationIdleMode) {
    return;
  }
  if (reason === "dialogue") {
    setTopMessage("Termine d'abord le dialogue.", 1200);
    return;
  }
  if (reason === "tutorial") {
    setTopMessage("Termine d'abord le tuto.", 1200);
    return;
  }
  if (reason === "trainer_battle") {
    setTopMessage("Menu indisponible pendant ce combat.", 1200);
  }
}

function setShopOpen(open) {
  if (open && isTrainerBattleActive()) {
    showPanelBlockMessage("trainer_battle");
    return;
  }
  if (open && state.ui.dialogueOpen) {
    showPanelBlockMessage("dialogue");
    return;
  }
  if (open && state.ui.tutorialOpen) {
    showPanelBlockMessage("tutorial");
    return;
  }
...
async function startGachaSpin(options = {}) {
  ...
  // In batch mode, the reel's final silhouette must match the first reward reveal.
  const reward = isBatchSpin
    ? (rewards[0] || rewards[rewards.length - 1] || null)
    : (rewards[rewards.length - 1] || rewards[0] || null);
...
const CANVAS_RUNTIME_SHELL_LEGACY_BINDING_KEYS = Object.freeze([
  "actionDockEl",
  "actionDockPokeballToggleButtonEl",
  "actionDockPokeballVisualEl",
  "coinsValueEl",
...
for (const bindingKey of CANVAS_RUNTIME_SHELL_LEGACY_BINDING_KEYS) {
  delete RUNTIME_BINDING_GETTERS[bindingKey];
}
RUNTIME_BINDING_GETTERS.openTopbarBallCaptureMenu = () => openTopbarBallCaptureMenu;
RUNTIME_BINDING_GETTERS.buildRouteNavigationViewModel = () => buildRouteNavigationViewModel;
RUNTIME_BINDING_GETTERS.getCurrentRouteZoneActions = () => zoneDialogueRuntime.getCurrentRouteZoneActions;
RUNTIME_BINDING_GETTERS.isActionDockFullscreenMenuOpen = () => isActionDockFullscreenMenuOpen;
RUNTIME_BINDING_GETTERS.isTrainerBattleActive = () => isTrainerBattleActive;
RUNTIME_BINDING_GETTERS.normalizeUiDisplayText = () => normalizeUiDisplayText;
...
RUNTIME_BINDING_GETTERS.toggleRouteNavDrawer = () => toggleRouteNavDrawer;
RUNTIME_BINDING_GETTERS.triggerZoneAction = () => triggerZoneAction;
RUNTIME_BINDING_GETTERS.EVOLUTION_ANIM_SWAP_COUNT = () => EVOLUTION_ANIM_SWAP_COUNT;
